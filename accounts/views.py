...
def auth_code(request):
    ...
    user.code = random.randint(10000, 100000)  # генерируем случайный код
    user.save()     # сохряняем данные пользователя, код хранится
    send_mail(
        subject=f'Notice Board: подтверждение e-mail',
        message=f'Hello, {request.user}. Для подтверждения регистрации, введите код {user.code} на '
