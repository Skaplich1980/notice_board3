...
from .tasks import respond_email, respond_accept_email
...
class CreatePost(LoginRequiredMixin, CreateView): # создание поста
    model = Post
...
class EditPost(PermissionRequiredMixin, UpdateView): # изменение поста
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.request.user.username == 'admin' or self.request.user.username == author:
            ...
            return HttpResponse("<h4><b>Только автор объявления может его редактировать!</b></h4>")
...
@login_required
def response_accept(request, **kwargs): # принятие отклика
    if request.user.is_authenticated:
        response = Response.objects.get(id=kwargs.get('pk'))
        response.status = True
        response.save()
        respond_accept_email.delay(response_id=response.id) # высылаем письмо по принятию отклика
        return HttpResponseRedirect('/responses')
    else:
        return HttpResponseRedirect('/accounts/login') # если не авторизирован пользователь, то переходим на страницу авторизации
...
class Respond(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        respond.save()
        respond_email.delay(res_id=respond.id)
